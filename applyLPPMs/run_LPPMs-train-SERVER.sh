...
tri="tri-train-$nameOutput-split-$split-range-$trl"
mkdir "$tri"
cd ..
...
possibleLog="$workdir""//log-XXXXXXXXXXX.txt"
 ...
 ##1- apply GEOI (LPPM)
	...
	path_geoi=$(bash "$sc/"getPath.sh $loggeoi $workdir "GeoIndistinguishability/data")
 	echo "$path_geoi"
	python "$sc/"formatData_To_FeatureVectors.py  "$path_geoi" $level  "$workdir/geoi-train-$nameOutput-split-$split-level-$level.csv" 
    ...
    bash "$sc/"getCsv.sh $log_sdistgeoi $workdir "SpatioTemporalDistortiongeoi/avg" "$workdir/train-dist-spatio-temporal-geoi-$geoi.csv"
    ...
    bash "$sc/"getCsv.sh $log_sdistgeoi2 $workdir "AreaCoverageMatrix/writtenStats" "$workdir/train-dist-area-coverage-geoi-$geoi.csv"
 ...
 ##3- apply TRL (LPPM)
	echo "********Trilateration********"
	pathTri="$workdir/tri-train-$nameOutput-split-$split-range-$trl/"
	echo "$urlData"
	echo "$pathTri"
	java -jar "$sc/"trilaterationv3.jar "$data/" $pathTri 0.6 $trl
	python "$sc/"formatData_To_FeatureVectors.py  $pathTri $level  "$workdir/tri-train-$nameOutput-split-$split-level-$level-range-$trl.csv" 
 ...
 ## we measure the spatial temporal distortion using accio:
    ...
    urlTestTri=$(realpath "$workdir/tri-accio")
        ...
        bash "$sc/"getCsv.sh $log_sdist $workdir "SpatioTemporalDistortiongeoi/avg" "$workdir/train-dist-spatio-temporal-trl-$trl.csv"
	...
	echo "**************Distortion2 measuring*****************"
        ...
        bash "$sc/"getCsv.sh $log_sdist2 $workdir "AreaCoverageMatrix/writtenStats" "$workdir/train-dist-area-coverage-trl-$trl.csv"
